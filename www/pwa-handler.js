...
" role="dialog" aria-modal="true" aria-labelledby="pwaInstallTitle">
  ...
  ">
    ...
    updateInstallButtonVisibility() {
        ...
        const isIos = this.platform.type === 'ios';
        const isSafariDesktop = (this.platform.type === 'pc' && this.platform.browser === 'safari');
        const shouldShowForSafari = (isIos || isSafariDesktop) && !hasPrompt && !isStandalone;

        const shouldShow = !isStandalone && (hasPrompt || shouldShowForSafari);

        if (shouldShow) {
            btn.classList.remove('hidden');
            // ปรับไอคอน/ข้อความให้เหมาะสม
            if ((isIos || isSafariDesktop) && !hasPrompt) {
                // บน Safari ที่ยังไม่มี prompt ให้ใช้ไอคอนแชร์
                btn.innerHTML = '<i class="fas fa-share-alt"></i>';
                btn.title = 'ติดตั้งแอป (เพิ่มลงหน้าจอหลัก)';
            } else {
