// ui-display.js v3.6.5 - Fixed Melody1/Melody2 name in InfoPanel (read from user state)
window.UI_VERSION = '3.6.5';
...
window.UI = {
    ...
    // ── ดึง defaultDNA และ customDNA จาก cache หรือ state ──
    const defaultDNA = this._cachedDefaultDNA
        || window.AudioController?._defaultDNA
        || null;
    const customDNA  = this._cachedCustomDNA
    ...
    // ✅ แก้ไข: รับชื่อแนวเพลงจาก user.musicPreference (เก็บตอนบันทึกฟอร์ม)
    let musicStyle1 = '-';
    let musicStyle2 = '-';
    try {
    ...
    // เริ่มสร้าง HTML (เหมือนเดิมทุกประการ)
    ...
    // --- Current Melody Settings (ใช้ชื่อจาก musicPreference) ---
    infoHTML += `
